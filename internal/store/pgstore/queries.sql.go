...
const getRoomMessagens = `-- name: GetRoomMessagens :many
SELECT 
    "id", "room_id", "message", "reaction_count", "answered"
FROM messages
WHERE "room_id" = $1
...
func (q *Queries) GetRoomMessagens(ctx context.Context, roomID uuid.UUID) ([]Message, error) {
	rows, err := q.db.Query(ctx, getRoomMessagens, roomID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const reacToMessage = `-- name: ReacToMessage :one
UPDATE messages
SET 
...
func (q *Queries) ReacToMessage(ctx context.Context, id uuid.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, reacToMessage, id)
